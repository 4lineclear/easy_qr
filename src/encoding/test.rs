use crate::{encoding::Encodable, ErrorCorrection, QRCodeVersion};
...
macro_rules! test {
    (
        ...
        //
        $(
            match $s.create_bits($version, $ec) {
                Some((bytes, _)) => {
                    assert_eq! (
                        bytes,
                        $a,
...
fn limit_numeric() {
    ...
    test![QRCodeVersion::V40, ErrorCorrection::L, &nines, encoded];
}

#[test]
fn limit_alphanumeric() {
    ...
    test![QRCodeVersion::V40, ErrorCorrection::L, &colons, encoded];
}
...
fn limit_byte() {
    ...
    test![QRCodeVersion::V40, ErrorCorrection::L, &chars, encoded];
}
